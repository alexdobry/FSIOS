...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, performFetchWithCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        let market = Market(baseCurrency: "USDT", logoUrl: nil, currency: "", currencyLong: "", name: "USDT-BTC", active: true) // FIXME: replace with user decision
        
        let service = MarketSummaryService.default(with: market)
        service.marketSummaryDelta { delta in
            if let delta = delta {
