...
class MatchingCardGame {
    ...
    func redrawCards() {
        ...
        if cardsOnBoard >= deck.cards.count {
            fatalError("numberOfCards must be lower than \(deck.cards.count)")
        } else {
            for _ in (0..<cardsOnBoard) {
                let card = deck.drawRandomCard()!
                cards.append(card)
            }
        }
    ...
    func revealCard(at index: Int) -> MatchingGameResult {
        guard isMatchingPossible else {
            delegate?.gameIsOver(to: score)
            return .gameOver(with: pendingCards) }
        ...
        if let previous = previousCard {
            ...
            if match {
                ...
                // EndOfGameBug
                if matchedCards.count == cardsOnBoard {
                    return .matchEnd(with: currentCard)
                }
    ...
    //Berechnung des Scores, wenn zwei Karten aufgedeckt, Rückgabe Score
