...
class PhotosCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, UIViewControllerPreviewingDelegate, PhotosCollectionViewCellDelete {
    ...
    //DONE
    var imageCount:CGFloat = 3 {
        didSet {
            if(imageCount < 1) {
                imageCount = 1
            }
            
            print("imageCount von \(oldValue) zu \(imageCount) gesetzt.")
            //collectionView?.indexPathsForVisibleItems
            collectionView?.reloadData()
        }
    }
    
    @IBAction func showlessViews(_ sender: Any) {
        imageCount -= 1
    }
    
    @IBAction func showMoreViews(_ sender: Any) {
        imageCount += 1
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        //edit Number with +/- buttons
        let numberOfItemsPerRow = imageCount //3
        let widthWithoutSpacing = collectionView.frame.width - (imageCount * CGFloat(numberOfItemsPerRow - 1)) //3
