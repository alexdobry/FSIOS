...
class SettingsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        savedTheme = UserDefaults.standard.string(forKey: "myTheme")!
        ...
        let theme = Theme.all.filter { $0.title == savedTheme }
        ThemeManager.current = theme[0]
    }
    ...
    @IBAction func cancel(_ sender: Any) {
        ...
        let current = ThemeManager.current // set last selected
        UserDefaults.standard.set(current.title, forKey: "myTheme")
        
        print("UserDefaults: \(UserDefaults.standard.string(forKey: "myTheme") ?? "- UD: Fehler")")
