...
public final class MarketSummaryService: DataProvider {
    
    private let market: Market
    private let webservice: Webservice
    private let database: Database?
    
    public static func with(market: Market) -> MarketSummaryService {
    ...
    public func model(from source: ModelSource, completion: @escaping (Result<[MarketSummary]>) -> Void) {
        switch source {
        case .network(let storage): fromNetwork(withStorage: storage, completion)
        case .database: fromDatabase(completion)
        case .both: fromDatabase(completion); fromNetwork(withStorage: true, completion)
        }
    }
    ...
    private func fromNetwork(withStorage: Bool, _ completion: @escaping (Result<[MarketSummary]>) -> Void) {
        webservice.get { (result: Result<MarketSummaryResult>) in
            ...
            case let .success(s):
                ...
                if withStorage, let all = self.database?.insert(first) {
    ...
    public func deltaFromNetwork(completion: @escaping (MarketSummaryDelta?) -> Void) {
        fromNetwork(withStorage: true) { result in
            let delta = result
