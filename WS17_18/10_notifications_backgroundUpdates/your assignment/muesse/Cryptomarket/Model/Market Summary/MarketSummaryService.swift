...
final class MarketSummaryService: DataProvider {
    ...
    func deltaFromNetwork(completion: @escaping (MarketSummaryDelta?) -> Void) {
        fromNetwork { result in
            let delta = result.map { $0.sorted(by: { $0.timestamp > $1.timestamp }) }.mapOrNil { summaries -> MarketSummaryDelta? in
                guard let first = summaries[safe: 0],
                    let second = summaries[safe: 1]
                else { return nil }
